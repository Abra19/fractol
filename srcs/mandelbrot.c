...
/*   Updated: 2020/06/09 01:01:08 by elena_abrarova   ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "fractol.h"

int		calc_mandelbrot(t_fract *fract, t_fract_m *fract_m)
{
	int		k;
	float	tmpx;
	float	tmpy;
...
void	draw_mandelbrot(t_fract *fract, t_fract_m *fract_m)
{
	int		i;
	int		j;
	int		k;
	...
	while (j < fract_m->f_height)
	{
		...
		while (i < fract_m->next)
		{
			...
			k = calc_mandelbrot(fract, fract_m);
...
void	mandelbrot(t_fract_m *fract, int n)
{
	...
	{
		fract->n_win = 1;
		draw_mandelbrot(&fract->fract1, fract);
	}
	if (n == 2)
	{
		fract->n_win = 2;
		draw_mandelbrot(&fract->fract2, fract);
	}
}
