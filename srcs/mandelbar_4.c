...
/*   Updated: 2020/04/04 21:01:57 by elena_abrarova   ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "fractol.h"

int		calc_mandelbar_4(t_fract *fract, t_fract_m *fract_m)
{
	int		k;
	float	tmpx;
	float	tmpy;
	...
	while (k < fract->iter)
	{
		...
		{
			tmpx = (-fract_m->xk2 + 3 * fract_m->yk2) * fract_m->xk + fract->p;
			tmpy = (3 * fract_m->xk2 - fract_m->yk2) * fract_m->yk + fract->q;
			fract_m->xk = tmpx;
			fract_m->yk = tmpy;
...
void	draw_mandelbar_4(t_fract *fract, t_fract_m *fract_m)
{
	int		i;
	int		j;
	int		k;
	...
	while (j < fract_m->f_height)
	{
		...
		while (i < fract_m->next)
		{
			...
			k = calc_mandelbar_4(fract, fract_m);
...
void	mandelbar_4(t_fract_m *fract, int n)
{
	...
	{
		fract->n_win = 1;
		draw_mandelbar_4(&fract->fract1, fract);
	}
	if (n == 2)
	{
		fract->n_win = 2;
		draw_mandelbar_4(&fract->fract2, fract);
	}
}
