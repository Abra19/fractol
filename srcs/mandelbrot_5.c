...
/*   Updated: 2020/04/04 19:55:17 by elena_abrarova   ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "fractol.h"

int		calc_mandelbrot_5(t_fract *fract, t_fract_m *fract_m)
{
	int		k;
	float	tmpx;
	float	tmpy;
	...
	while (k < fract->iter)
	{
		...
		{
			tmpx = fract_m->xk4 * fract_m->xk -\
				10 * fract_m->xk2 * fract_m->xk * fract_m->yk2 +\
				5 * fract_m->xk * fract_m->yk4 + fract->p;
			tmpy = fract_m->yk * (5 * fract_m->xk4 -\
				10 * fract_m->xk2 * fract_m->yk2 + fract_m->yk4) + fract->q;
			fract_m->xk = tmpx;
			fract_m->yk = tmpy;
...
void	draw_mandelbrot_5(t_fract *fract, t_fract_m *fract_m)
{
	int		i;
	int		j;
	int		k;
	...
	while (j < fract_m->f_height)
	{
		...
		while (i < fract_m->next)
		{
			...
			k = calc_mandelbrot_5(fract, fract_m);
...
void	mandelbrot_5(t_fract_m *fract, int n)
{
	...
	{
		fract->n_win = 1;
		draw_mandelbrot_5(&fract->fract1, fract);
	}
	if (n == 2)
	{
		fract->n_win = 2;
		draw_mandelbrot_5(&fract->fract2, fract);
	}
}
