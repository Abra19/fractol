...
/*   Updated: 2020/04/04 20:56:31 by elena_abrarova   ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "fractol.h"

int		calc_julia_4(t_fract *fract, t_fract_m *fract_m)
{
	int		k;
	float	tmpx;
	float	tmpy;
	...
	while (k < fract->iter)
	{
		...
		{
			tmpx = fract_m->p4 - 6 * fract_m->p2 * fract_m->q2 + fract_m->q4 +\
			fract->cx;
			tmpy = 4 * fract_m->p2 * fract->p * fract->q -\
			4 * fract_m->q2 * fract->q * fract->p + fract->cy;
			fract->p = tmpx;
			fract->q = tmpy;
...
void	draw_julia_4(t_fract *fract, t_fract_m *fract_m)
{
	int		i;
	int		j;
	int		k;
...
void	julia_4(t_fract_m *fract, int n)
{
	...
	{
		fract->n_win = 1;
		draw_julia_4(&fract->fract1, fract);
	}
	if (n == 2)
	{
		fract->n_win = 2;
		draw_julia_4(&fract->fract2, fract);
	}
}
