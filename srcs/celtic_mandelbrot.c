...
/*                                                                            */
/*                                                        :::      ::::::::   */
/*  celtic_ mandelbrot.c                                :+:      :+:    :+:   */
/*                                                    +:+ +:+         +:+     */
/*   By: dlaure <marvin@42.fr>                      +#+  +:+       +#+        */
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2020/02/29 19:21:11 by dlaure            #+#    #+#             */
/*   Updated: 2020/04/04 20:44:27 by elena_abrarova   ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "fractol.h"

int		celtic_mandelbrot_calc(t_fract *fract, t_fract_m *fract_m)
{
	int		k;
	float	tmpx;
	float	tmpy;
	...
	while (k < fract->iter)
	{
		...
		{
			tmpx = ft_abs_float(fract_m->xk2 - fract_m->yk2) + fract->p;
...
void	draw_celtic_mandelbrot(t_fract *fract, t_fract_m *fract_m)
{
	int		i;
	int		j;
	int		k;
	...
	while (j < fract_m->f_height)
	{
		...
		while (i < fract_m->next)
		{
			...
			k = celtic_mandelbrot_calc(fract, fract_m);
...
void	celtic_mandelbrot(t_fract_m *fract, int n)
{
	...
	{
		fract->n_win = 1;
		draw_celtic_mandelbrot(&fract->fract1, fract);
	}
	if (n == 2)
	{
		fract->n_win = 2;
		draw_celtic_mandelbrot(&fract->fract2, fract);
	}
}
